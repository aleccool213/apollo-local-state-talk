...
<Image src="https://res.cloudinary.com/dscgr6mcw/image/upload/v1574823654/apollo-state-talk/download_1.jpg" width="50%" height="80%" />
...
### What if we are want to use GraphQL?

### What if we are want to use Apollo Client?

### What if we don't want to bring in another state management library?
...
```diff 3:7 title="Define the mutation"
```

```diff 5[36:43] title="New directive!"
```

```diff 12,16:25 title="Use the mutation"
```
...
```diff 1[1:6] title="Apollo Client will merge this schema with the remote one"
```
...
```ts file=./mutations/updateIssueName.ts title="Next up, the resolver" subtitle="./resolvers/updateGithubIssueName.ts"
```

```diff 11:17 title="Define fragment"
```

```diff 36:40,50:55 title="Get/Write the data from the cache"
```

```diff 45:48 title="Update the data"
```

```diff 1:9,20:34,41:43,56 title="Other stuff"
```
...
```ts file=./mutations/updateIssueName.ts 31:34,50:55
```
...
```ts file=./mutations/updateIssueName.ts 45:48
```

</CodeSurfer>

Isn't this part familar 🤔
